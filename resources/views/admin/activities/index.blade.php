...
@section('content')
    <div class="max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-4">
            ...
            <x-mm-button href="{{ route('admin.activities.create') }}" variant="accent" label="Add activity"
        ...
        <form method="GET" class="mb-4" onsubmit="return false;">
            <div class="flex gap-2">
                <div class="relative w-full">
                    ...
                    <x-mm-input name="q" id="admin_activities_q" :value="$q ?? ''" placeholder="Search name/code"
                        class="pl-10 pr-3 py-2 rounded-lg bg-transparent" autocomplete="off" />

                    <div id="admin-activities-search-results"
                        class="absolute left-0 w-full mt-2 rounded bg-mm-accent border border-mm-border z-50 hidden"
                        style="display: none;">
                        <div id="admin-activities-search-list" class="divide-y divide-gray-700 max-h-80 overflow-auto">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto relative border border-mm-border bg-mm-card shadow-sm rounded-lg">
                    ...
                    @foreach ($activities as $activity)
                        <tr class="border-t border-mm-border">
                            <td class="px-4 py-3 align-middle">
                                <div>
                                    <div class="font-medium text-white">{{ $activity->ma_name }}</div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 align-middle">
                                <div class="flex items-center gap-2">
                                    <x-mm-button href="{{ route('admin.activities.edit', $activity->ma_code) }}"
                                        label="Edit" variant="secondary" class="px-3 py-1 text-sm" />
            ...
            <div class="p-4 flex items-center justify-between">
                <div class="text-mm-gray">{{ $activities->total() }} results</div>
                <div>
                    {{ $activities->links() }}
                </div>
            </div>
...
@push('scripts')
    <script>
        (function() {
            ...
            const results_panel = document.getElementById('admin-activities-search-results');
            const results_list = document.getElementById('admin-activities-search-list');
